...
public class Cours extends JFrame {
	...
	public Cours(){
		...
		lbtitre=new JLabel("Opérations sur table enseignants");
		lbtitre.setBounds(20,10,400,30);
		lbtitre.setFont(new Font("Arial",Font.BOLD,18));
		pn.add(lbtitre);
		
		lbtitre2=new JLabel("Opérations sur table cours");
		...
		btrech.addActionListener(new ActionListener(){
		    public void actionPerformed(ActionEvent ev){
		        ...
		        if (!matricule.isEmpty()) {
		            // Si le champ de matricule n'est pas vide, on recherche par matricule
		            rq = "SELECT * FROM tb_enseignant WHERE matricule = '" + matricule + "'";
		        } else {
		            // Sinon, on recherche par nom
		            rq = "SELECT * FROM tb_enseignant WHERE nom LIKE '%" + nom + "%'";
		        }
		        ...
		        try {
		            ...
		            if (rst.next()) {
		                ...
		                JOptionPane.showMessageDialog(null, "Enregistrement inexistant pour la recherche spécifiée !", null, JOptionPane.ERROR_MESSAGE);
		            }
		        } catch(SQLException ex) {
		            JOptionPane.showMessageDialog(null, "Erreur d'exécution de la requête SQL !", "Erreur SQL", JOptionPane.ERROR_MESSAGE);   
		        }
		    }
		});
		...
		//Classe
				...
				comboclasse.addItem("TIC1E");
				comboclasse.addItem("TIC1F");
	...
	btenrg.addActionListener(new ActionListener(){
		public void actionPerformed(ActionEvent ev){
			...
			try{
	    		...
	    		JOptionPane.showMessageDialog(null,"Insertion reussie!","Succès",JOptionPane.INFORMATION_MESSAGE);
				}
				else{
					JOptionPane.showMessageDialog(null,"Completez le formulaire!","Erreur de Formulaire",JOptionPane.ERROR_MESSAGE);
				}
			}
			catch(SQLException ex){
		    	JOptionPane.showMessageDialog(null,"Erreur ! Déjà existant ", null,JOptionPane.ERROR_MESSAGE);	
		    }
			dispose();
			Cours crs=new Cours();
			crs.setVisible(true);
		...
		//bouton modification enseignant
			...
			try{
				...
				if(!matricule.equals("")&&!nom.equals("")&&!contact.equals("")){
				st.executeUpdate(rq);
	    		JOptionPane.showMessageDialog(null,"Modification réussie !", "Succès",JOptionPane.INFORMATION_MESSAGE);
				}
				else{
					JOptionPane.showMessageDialog(null,"Complétez le formulaire !",null,JOptionPane.ERROR_MESSAGE);
				}
			}
			catch(SQLException ex){
		    	JOptionPane.showMessageDialog(null,"Erreur d'exécution de la requête SQL !", null,JOptionPane.ERROR_MESSAGE);	
		    }	
			dispose();
			Cours crs=new Cours();
			crs.setVisible(true);	}
		...
		//bouton suppression enseignant
			...
			try{
				st=con.laConnection().createStatement();
				if(JOptionPane.showConfirmDialog(null,"Voulez vous supprimer? ", "Confirmation",JOptionPane.OK_CANCEL_OPTION)==JOptionPane.OK_OPTION){
				st.executeUpdate(rq);
	    		JOptionPane.showMessageDialog(null,"Suppression reussie!","Succès",JOptionPane.INFORMATION_MESSAGE);
				}
